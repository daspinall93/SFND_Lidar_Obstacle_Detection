...
void simpleHighway(pcl::visualization::PCLVisualizer::Ptr &viewer)
{
    ...
    std::shared_ptr<Lidar> lidar(new Lidar(cars, groundSlope));

    // Generate scan and render
    const auto scan = lidar->scan();
    // renderRays(viewer, lidar->position, scan);
    renderPointCloud(viewer, scan, "PointCloud");

    ProcessPointClouds<pcl::PointXYZ> processor;

    // Segment plane from rest of point cloud
    auto segmentCloud = processor.SegmentPlane(scan, 100, 0.2);
    // renderPointCloud(viewer,segmentCloud.first,"obstCloud",Color(1,0,0));
    // renderPointCloud(viewer, segmentCloud.second, "planeCloud", Color(0, 1, 0));

    // Cluster other points that aren't part of the ground plane
...
int main(int argc, char **argv)
{
    ...
    auto stream = processor.streamPcd("../src/sensors/data/pcd/data_1");
    ...
    while (!viewer->wasStopped())
    {
        ...
        auto inputCloud = processor.loadPcd((*streamIt).string());
